...
Run agentignore-check on every pull request.

```yaml
name: agentignore-check
  ...
  check:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 20
      - run: npx github:aolingge/agentignore-check --path fixtures/good.txt --min-score 80 --annotations
```
